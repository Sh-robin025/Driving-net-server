...
//     topBannerCollection.find().toArray((err, items) => res.send(items));
//   });
...
//     serviceCollection.find().toArray((err, items) => res.send(items));
//   });
...
//     courseCollection.find().toArray((err, items) => res.send(items));
//   });
...
//     adminCollection.find({ email: req.body.body.email }).toArray((err, admin) => {
//       if (admin.length === 0) {
//         userCollection
//           .find({ "data.email": { $exists: true } }, { "data.email": req.body.body.email })
//           .toArray((err, result) => {
//             if (result.length === 0) {
//               userCollection
//                 .insertOne({ data: req.body.body, isCustomer: true })
//                 .then((result) => result.insertedCount > 0 && res.sendStatus(200))
...
//     adminCollection.find({ email: req.params.email }).toArray((err, admin) => {
//       admin.length !== 0 && res.send(admin);
//       if (admin.length === 0) {
//         userCollection
//           .find({ "data.email": { $exists: true } }, { "data.email": req.params.email })
...
//     adminCollection.find({ email: req.params.email }).toArray((err, result) => {
//       if (result.length) {
//         orderCollection.find().toArray((err, items) => res.send(items));
//       } else {
//         orderCollection.find({ email: req.params.email }).toArray((err, items) => res.send(items));
//       }
//     });
//   });
// });
